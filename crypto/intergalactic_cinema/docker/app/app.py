...
def get_user_input_values():
    if 'user_input_values' not in session:
        session['user_input_values'] = {char: char for char in combined_array}
    return session['user_input_values']
...
def index():
    ...
    for i, char in enumerate(combined_array):
        ...
        table_html += f'<td>{char}</td><td><input type="text" name="{char}" value="{get_user_input_values()[char]}" maxlength="1"></td>'
        if i % 4 == 3:
            table_html += '</tr>'
    table_html += '</table>'
    
    # Render the updated encrypted text if available
    updated_text = session.get('updated_text', '')
...
def update_mapping():
    ...
    for char in combined_array:
        ...
        # Update user input values in session
        get_user_input_values()[char] = new_mapping[char]
    
    # Update the encrypted text with the new mapping
    update_encrypted_text(new_mapping)
    
    # Set session variable for updated text
    session['updated_text'] = 'Updated Encrypted Text:'
